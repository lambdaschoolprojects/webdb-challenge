...
> A RDBMS is a type of database that uses tables and relationships to manage data. SQL is an implementation of an RDBMS.
1. Why do tables need a `primary key`?
> The primary key is a unique key that allows you to access a specific row on a table. It ensures that all data can be accessed
1. What is the name given to a table column that references the primary key on another table.
> Foreign key
1. What do we need in order to have a _many to many_ relationship between two tables.
> We need a third table that has a column for a foreign key for each of the related tables
